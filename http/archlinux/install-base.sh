...
/usr/bin/pacstrap ${TARGET_DIR} base sudo base-devel
/usr/bin/arch-chroot ${TARGET_DIR} pacman -S --noconfirm gptfdisk openssh xe-guest-utilities syslinux
/usr/bin/arch-chroot ${TARGET_DIR} syslinux-install_update -i -a -m
/usr/bin/sed -i 's/sda3/xvda3/' "${TARGET_DIR}/boot/syslinux/syslinux.cfg"
/usr/bin/sed -i 's/sda1/xvda1/' "${TARGET_DIR}/boot/syslinux/syslinux.cfg"
/usr/bin/sed -i 's/xvda3/xvda1/' "${TARGET_DIR}/boot/syslinux/syslinux.cfg"
...
cat <<-EOF > "${TARGET_DIR}${CONFIG_SCRIPT}"
	...
	/usr/bin/ln -s '/usr/lib/systemd/system/dhcpcd@.service' '/etc/systemd/system/multi-user.target.wants/dhcpcd@eth1.service'
